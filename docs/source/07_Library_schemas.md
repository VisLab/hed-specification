...
- An unmerged schema should only include content in the auxiliary sections (unit classes, units, unit modifiers, value classes, and schema attributes) and the extra sections (Sources, Prefixes, and External annotations) that it explicitly extends. Otherwise, the section is present and empty except for its section header in that format. Every section is written; see [3.1.4.10. Sources, prefixes, and external annotations](./03_HED_formats.md#31410-sources-prefixes-and-external-annotations) for the files a reader must accept without the extra sections. The merged version will have all of the section values from both the library schema and its partner.
...
| `['testaux_1.0.0', 'testclash_17.0.0']`   | schema attribute `auxAttribute` | Declared without `boolRange`: for schema attribute elements the properties are the attribute values that are compared.                                                                 | No    |
