...
Merge groups (i.e., namespaces) are defined in the schema version specification not in the schema itself. A given schema can be combinedThe schemas listed in the default namespace (no prefix) form one merge group, and each unique prefix in the version specification designates a distinct merge group. For example the following [BIDS](https://bids.neuroimaging.io/index.html) version specification:
...
| Element compatibility | An element appearing in more than one schema of a merge group must either have a `#` child in all of appearances, or in none of them. Whne the `#` is present, it must have the same attributes in all schemas.                   |
